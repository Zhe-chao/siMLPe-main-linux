...
# Ensure the shared lib directory is available ahead of site packages.
...
def _adapt_state_dict_keys(state_dict):
    if not any(k.startswith("motion_transformer.transformer") for k in state_dict.keys()):
        return state_dict
    from collections import OrderedDict
    new_state = OrderedDict()
    for key, value in state_dict.items():
        if key.startswith("motion_transformer.transformer"):
            new_key = key.replace("motion_transformer.transformer", "motion_mlp.mlps", 1)
        else:
            new_key = key
...
def train_step(amass_motion_input, amass_motion_target, model, optimizer, nb_iter, total_iter, max_lr, min_lr) :
    ...
    if config.deriv_input:
        ...
        amass_motion_input_ = torch.matmul(dct_m, amass_motion_input_.to(DEVICE))
    else:
        amass_motion_input_ = amass_motion_input.clone()

    motion_pred = model(amass_motion_input_.to(DEVICE))
    motion_pred = torch.matmul(idct_m, motion_pred)

    if config.deriv_output:
        offset = amass_motion_input[:, -1:].to(DEVICE)
    ...
    amass_motion_target = amass_motion_target.to(DEVICE).reshape(b,n,18,3).reshape(-1,3)
...
if config.model_pth is not None :
    state_dict = torch.load(config.model_pth, map_location=DEVICE)
